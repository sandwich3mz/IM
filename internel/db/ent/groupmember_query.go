...
type GroupMemberQuery struct {
	...
	withGroupUser   *UserQuery
	withMemberGroup *GroupQuery
...
func (gmq *GroupMemberQuery) QueryGroupUser() *UserQuery {
...
func (gmq *GroupMemberQuery) QueryMemberGroup() *GroupQuery {
...
func (gmq *GroupMemberQuery) Clone() *GroupMemberQuery {
	...
	return &GroupMemberQuery{
		...
		withGroupUser:   gmq.withGroupUser.Clone(),
		withMemberGroup: gmq.withMemberGroup.Clone(),
...
func (gmq *GroupMemberQuery) WithGroupUser(opts ...func(*UserQuery)) *GroupMemberQuery {
	query := (&UserClient{config: gmq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	gmq.withGroupUser = query
	return gmq
}

// WithMemberGroup tells the query-builder to eager-load the nodes that are connected to
// the "member_group" edge. The optional arguments are used to configure the query builder of the edge.
func (gmq *GroupMemberQuery) WithMemberGroup(opts ...func(*GroupQuery)) *GroupMemberQuery {
	query := (&GroupClient{config: gmq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	gmq.withMemberGroup = query
	return gmq
}
...
func (gmq *GroupMemberQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*GroupMember, error) {
	var (
		...
		_spec       = gmq.querySpec()
		loadedTypes = [2]bool{
			gmq.withGroupUser != nil,
			gmq.withMemberGroup != nil,
		}
	)
	...
	if query := gmq.withGroupUser; query != nil {
		if err := gmq.loadGroupUser(ctx, query, nodes, nil,
			func(n *GroupMember, e *User) { n.Edges.GroupUser = e }); err != nil {
			return nil, err
		}
	}
	if query := gmq.withMemberGroup; query != nil {
		if err := gmq.loadMemberGroup(ctx, query, nodes, nil,
			func(n *GroupMember, e *Group) { n.Edges.MemberGroup = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (gmq *GroupMemberQuery) loadGroupUser(ctx context.Context, query *UserQuery, nodes []*GroupMember, init func(*GroupMember), assign func(*GroupMember, *User)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*GroupMember)
	for i := range nodes {
		fk := nodes[i].UserID
...
func (gmq *GroupMemberQuery) loadMemberGroup(ctx context.Context, query *GroupQuery, nodes []*GroupMember, init func(*GroupMember), assign func(*GroupMember, *Group)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*GroupMember)
	for i := range nodes {
		fk := nodes[i].GroupID
...
func (gmq *GroupMemberQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := gmq.ctx.Fields; len(fields) > 0 {
		...
		if gmq.withGroupUser != nil {
			_spec.Node.AddColumnOnce(groupmember.FieldUserID)
		}
		if gmq.withMemberGroup != nil {
