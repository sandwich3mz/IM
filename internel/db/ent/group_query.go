...
type GroupQuery struct {
	...
	withGroupMember *GroupMemberQuery
...
// QueryGroupMember chains the current query on the "group_member" edge.
func (gq *GroupQuery) QueryGroupMember() *GroupMemberQuery {
...
func (gq *GroupQuery) Clone() *GroupQuery {
	...
	return &GroupQuery{
		...
		withGroupMember: gq.withGroupMember.Clone(),
...
// WithGroupMember tells the query-builder to eager-load the nodes that are connected to
// the "group_member" edge. The optional arguments are used to configure the query builder of the edge.
func (gq *GroupQuery) WithGroupMember(opts ...func(*GroupMemberQuery)) *GroupQuery {
	query := (&GroupMemberClient{config: gq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	gq.withGroupMember = query
	return gq
}
...
func (gq *GroupQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Group, error) {
	...
	if query := gq.withGroupMember; query != nil {
		if err := gq.loadGroupMember(ctx, query, nodes,
			func(n *Group) { n.Edges.GroupMember = []*GroupMember{} },
			func(n *Group, e *GroupMember) { n.Edges.GroupMember = append(n.Edges.GroupMember, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (gq *GroupQuery) loadGroupMember(ctx context.Context, query *GroupMemberQuery, nodes []*Group, init func(*Group), assign func(*Group, *GroupMember)) error {
	...
	query.Where(predicate.GroupMember(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(group.GroupMemberColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
