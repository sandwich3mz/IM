...
func (fq *FriendQuery) QueryOwnerUser() *UserQuery {
...
// QueryFriendUser chains the current query on the "friend_user" edge.
func (fq *FriendQuery) QueryFriendUser() *UserQuery {
	query := (&UserClient{config: fq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := fq.prepareQuery(ctx); err != nil {
			return nil, err
...
func (fq *FriendQuery) WithOwnerUser(opts ...func(*UserQuery)) *FriendQuery {
	query := (&UserClient{config: fq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	fq.withOwnerUser = query
	return fq
}

// WithFriendUser tells the query-builder to eager-load the nodes that are connected to
// the "friend_user" edge. The optional arguments are used to configure the query builder of the edge.
func (fq *FriendQuery) WithFriendUser(opts ...func(*UserQuery)) *FriendQuery {
	query := (&UserClient{config: fq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (fq *FriendQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Friend, error) {
	...
	if query := fq.withOwnerUser; query != nil {
		if err := fq.loadOwnerUser(ctx, query, nodes, nil,
			func(n *Friend, e *User) { n.Edges.OwnerUser = e }); err != nil {
			return nil, err
		}
	}
...
func (fq *FriendQuery) loadOwnerUser(ctx context.Context, query *UserQuery, nodes []*Friend, init func(*Friend), assign func(*Friend, *User)) error {
...
func (fq *FriendQuery) loadFriendUser(ctx context.Context, query *UserQuery, nodes []*Friend, init func(*Friend), assign func(*Friend, *User)) error {
	ids := make([]int64, 0, len(nodes))
	nodeids := make(map[int64][]*Friend)
	for i := range nodes {
		fk := nodes[i].FriendUserID
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "friend_user_id" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (fq *FriendQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := fq.ctx.Fields; len(fields) > 0 {
		...
		if fq.withOwnerUser != nil {
			_spec.Node.AddColumnOnce(friend.FieldOwnerUserID)
		}
